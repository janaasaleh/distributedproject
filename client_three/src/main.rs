...
async fn main() {
    ...
    let client_socket = UdpSocket::bind("127.0.0.10:0")
        .await
    ...
    let middleware_task = tokio::spawn(async move {
            ...
            middleware_socket.recv_from(&mut buffer).await
        {
            println!("Yo1");
            println!("Yo2");
            let server_socket = UdpSocket::bind("127.0.0.10:0")
                .await
    ...
    // Client code here
    let client_message = "Request from Client 3!";
    let middleware_address = "127.0.0.10:12345"; // Replace with the actual middleware address and port
