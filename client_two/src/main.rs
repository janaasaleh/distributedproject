...
async fn main() {
    ...
    let client_socket = UdpSocket::bind("127.0.0.9:0").await.expect("Failed to bind client socket");
    ...
    let middleware_task = tokio::spawn(async move {
        if let Ok((bytes_received, client_address)) = middleware_socket.recv_from(&mut buffer).await {
            println!("Yo1");
            println!("Yo2");
            let mut server_socket = UdpSocket::bind("127.0.0.9:0").await.expect("Failed to bind server socket");
