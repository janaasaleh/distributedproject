            ...
            for (index, piece) in image_data.chunks(MAX_CHUNCK).enumerate() {
                ...
                if index == packet_number {
                    ...
                    if let Ok(encrypted_image) = image::load_from_memory(&encrypted_image_data) {
                        let (width, height) = encrypted_image.dimension();
